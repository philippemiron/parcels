...
from parcels.compilation import GNUCompiler
from parcels.application_kernels.advection import AdvectionRK4
from parcels.particleset import ParticleSetSOA
from parcels.particle import JITParticle
from parcels.kernel import KernelSOA_Benchmark, KernelSOA, Kernel
...
__all__ = ['ParticleSetSOA_Benchmark']
...
class ParticleSetSOA_Benchmark(ParticleSetSOA):
    ...
    def __init__(self, fieldset, pclass=JITParticle, lon=None, lat=None, depth=None, time=None, repeatdt=None,
        ...
        super(ParticleSetSOA, self).__init__(fieldset, pclass, lon, lat, depth, time, repeatdt, lonlatdepth_dtype, pid_orig, **kwargs)
    ...
    def execute(self, pyfunc=AdvectionRK4, pyfunc_inter=None, endtime=None, runtime=None, dt=1.,
        ...
        if self.kernel is None or (self.kernel.pyfunc is not pyfunc and self.kernel is not pyfunc):
            ...
            if self.collection.ptype.uses_jit:
                ...
                cppargs = ['-DDOUBLE_COORD_VARIABLES'] if self.collection.lonlatdepth_dtype == np.float64 else None
        ...
        while (time < endtime and dt > 0) or (time > endtime and dt < 0) or dt == 0:
            ...
            # ==== compute ==== #
            if not isinstance(self.kernel, KernelSOA_Benchmark):
                self.compute_log.start_timing()
            # ---- InteractioNKernel changes ---- #
            ...
            # E.g. Inter -> Normal -> Inter -> Normal if endtime-time == 2*dt
                ...
                while (cur_time < next_time and dt > 0) or (cur_time > next_time and dt < 0) or dt == 0:
                    ...
                    # OK, now I see: runs without any interaction are not affected. That said, this setup cripples the
                    # speed advantage of a normal JIT Kernel. But probably now I understand even more: mixing a normal
                    # JIT Kernel and a SciPy interaction kernel is not possible, so the speed drawback would anyway not be impactful.
                    self.kernel.execute(
                        self, endtime=cur_end_time, dt=dt, recovery=recovery,
                        output_file=output_file, execute_once=execute_once)
                    cur_time += dt
            ...
            if abs(time-next_prelease) < tol:
                ...
                if not isinstance(self.kernel, KernelSOA_Benchmark):
                    ...
                    pass
            if isinstance(self.kernel, KernelSOA_Benchmark):
    ...
    def Kernel(self, pyfunc, c_include="", delete_cfiles=True):
        ...
        """
        return KernelSOA_Benchmark(self.fieldset, self.ptype, pyfunc=pyfunc, c_include=c_include, delete_cfiles=delete_cfiles)
