...
from parcels.particleset import ParticleSetSOA, ParticleSetAOS
from parcels.kernel import KernelSOA, KernelAOS
from parcels.particlefile import ParticleFileSOA, ParticleFileAOS
...
pset_modes = ['soa', 'aos']
pset_mode = {
    'soa': {'pset': ParticleSetSOA, 'pfile': ParticleFileSOA, 'kernel': KernelSOA},
...
if __name__=='__main__':
    ...
    parser.add_argument("-pm", "--pset_mode_str", dest="pset_mode_str", choices=pset_modes, default='aos', help="Choice of the used ParticleSet structure (default: aos)")
    args = parser.parse_args()

    pset_mode_str = args.pset_mode_str
    # dryrun = 'b' not in pset_mode_str
    ParticleSet = pset_mode[pset_mode_str]['pset']
    ...
    # to be command-determined.
    # dt_minutes = 60  # X
    # dt_minutes = 20
    dt_seconds = 0.0025
    outdt_minutes = 7 * 24 *60.0
    #random.seed(123456)
    ...
    if backwardSimulation:
        # ==== backward simulation ==== #
        if args.animate:
            pset.execute(kernels, runtime=delta(days=time_in_days), dt=delta(minutes=-dt_seconds), output_file=output_file, recovery={ErrorCode.ErrorOutOfBounds: delete_func}, postIterationCallbacks=postProcessFuncs, callbackdt=delta(hours=12), moviedt=delta(hours=6), movie_background_field=fieldset.U)
        else:
            pset.execute(kernels, runtime=delta(days=time_in_days), dt=delta(minutes=-dt_seconds), output_file=output_file, recovery={ErrorCode.ErrorOutOfBounds: delete_func}, postIterationCallbacks=postProcessFuncs, callbackdt=delta(hours=12))
    else:
        # ==== forward simulation ==== #
        if args.animate:
            pset.execute(kernels, runtime=delta(days=time_in_days), dt=delta(minutes=dt_seconds), output_file=output_file, recovery={ErrorCode.ErrorOutOfBounds: delete_func}, postIterationCallbacks=postProcessFuncs, callbackdt=delta(hours=12), moviedt=delta(hours=6), movie_background_field=fieldset.U)
